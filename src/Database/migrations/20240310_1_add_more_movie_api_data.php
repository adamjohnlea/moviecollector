...
class Migration20240310_1 extends Migration
{
    public function up(\PDO $db): void
    {
        Logger::info("Running migration: AddMoreMovieApiData - up");
        
        try {
            // Add new columns for additional API data
            $db->exec("ALTER TABLE movies ADD COLUMN recommendations TEXT");
            $db->exec("ALTER TABLE movies ADD COLUMN videos TEXT");
            $db->exec("ALTER TABLE movies ADD COLUMN images TEXT");
            $db->exec("ALTER TABLE movies ADD COLUMN reviews TEXT");
            $db->exec("ALTER TABLE movies ADD COLUMN external_ids TEXT");
            
            Logger::info("Successfully added additional API data columns");
        } catch (\Exception $e) {
            Logger::error("Failed to add additional API data columns", [
                'error' => $e->getMessage(),
    ...
    {
        Logger::info("Running migration: AddMoreMovieApiData - down");
        ...
        try {
            ...
            $db->exec("
                INSERT INTO movies_old (
                    ...
                    adult, video, certifications, keywords, watch_providers, credits, similar,
            ...
            Logger::info("Successfully removed additional API data columns");
        } catch (\Exception $e) {
            Logger::error("Failed to remove additional API data columns", [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            throw $e;
